...
void read_cab_from_file(string cab_ifname, vector<double> &pa, vector<double> &cab, vector<double> &_rhoq, int Q)
{
	cout << "Reading pa,cab,rhoq from file... " << cab_ifname << flush;
	ifstream fin(cab_ifname.c_str());
	assert(fin.good() && "I can not open file which contains pa, cab and rhoq");
	string tmpstr;
	bool control_line = false;
	while (true)
	{
		if (!control_line)
		{ // control line is read, don't override it.
			if (!(fin >> tmpstr))
				break;
		}
		control_line = false;
		if (tmpstr == string("pa_begin") || tmpstr == string("#Vector_na:"))
		{
			...
			while (tmpstr != string("pa_end"))
			{
				...
				{ //
					...
					for (unsigned int i = 0; i < tmppa.size(); i++)
						pa.push_back(atof(tmppa[i].c_str()));
				}
				else
					pa.push_back(atof(tmpstr.c_str()));
				if (!(fin >> tmpstr))
					break;
			}
		}
		else if (tmpstr == string("cab_begin") || tmpstr == string("#Matrix_cab:"))
		{
			...
			while (tmpstr != string("cab_end"))
			{
				...
				{ //
					...
					for (unsigned int i = 0; i < tmpcab.size(); i++)
						cab.push_back(atof(tmpcab[i].c_str()));
				}
				else
					cab.push_back(atof(tmpstr.c_str()));
				if (!(fin >> tmpstr))
					break;
			}
		}
		else if (tmpstr == string("rhoq_begin") || tmpstr == string("#Vector_rhoq:"))
		{
			if (!(fin >> tmpstr))
				break;
			// cout<<tmpstr<<endl;
			while (tmpstr != string("rhoq_end"))
			{
				if (tmpstr[0] == '#')
				{
					control_line = true;
				...
				if (tmpstr.find(',') != string::npos)
				{ //
					vector<string> tmprhoq = strsplit(tmpstr, ",");
					for (unsigned int i = 0; i < tmprhoq.size(); i++)
						_rhoq.push_back(atof(tmprhoq[i].c_str()));
				}
				else
					_rhoq.push_back(atof(tmpstr.c_str()));
				if (!(fin >> tmpstr))
					break;
			}
		}
	...
	assert((_rhoq.size() == Q) && "there is something wrong in setting rhoq from file");
	fin.close();
	cout << "done" << endl;
}
//}}}
...
void show_help(char **argv)
{
	...
	cout << " -a: set rhoq, use ',' to separate." << endl;
	...
	cout << " -L: set cab_ifname, name of file which contains pa, cab and rhoq." << endl;
	...
	cout << endl
		 << "To generate a graph one needs to provide number of nodes using -n parameter, number of comminities using -q parameter and pa and cab matrix. The graph will be written in gml format into filename given by -w parameter or spm format give by -W parameter." << endl;
	cout << endl
		 << "There are four ways to set them vector pa and matrix cab:" << endl;
...
void parse_command_line(int argc, char **argv, string &lfname, double &learning_rate, double &dumping_rate, double &bp_conv_crit, double &learning_conv_crit, int &time_conv, int &Q, int &randseed, int &learning_max_time, string &pastr, string &cabstr, string &rhoqstr, int &infer_mode, string &eps_c_str, int &init_flag, int &N, string &afname, int &vflag, string &gml_fname, string &spm_fname, string &cab_ifname, string &marginal_fname, bool &no_learn_pa, int &wblock, double &wstep, string &Aij_fname, int &spcmode, int &dc_flag, int &LARGE_DEGREE, string &cab_ofname, int &genmode)
{
	...
	int option_index = 0;
	char cc;
	while ((cc = getopt_long(argc, argv, "l:r:R:e:E:t:q:D:v:s:L:c:I:T:p:P:i:m:n:w:W:v:d:M:A:a:h", long_options, &option_index)) != -1)
	{
		cout << "cc=" << cc << endl;
		...
		case 'r':
			...
			break;
		case 'R':
			dumping_rate = atof(optarg);
			break;
		case 'e':
			bp_conv_crit = atof(optarg);
			break;
		case 'E':
			learning_conv_crit = atof(optarg);
			break;
		case 't':
			time_conv = atoi(optarg);
			break;
		case 'T':
			learning_max_time = atoi(optarg);
			break;
		case 'p':
			pastr = optarg;
			break;
...
int main(int argc, char **argv)
{
	...
	if (argc == 1)
		show_help_short(argv);
	string function;
	if (argc > 1)
	{
		function = argv[1];
		if (function == string("-h") || function == string("--help"))
			show_help(argv);
		else if (function != string("gen") && function != string("infer") && function != string("learn") && function != string("walk") && function != string("spc"))
			show_help_short(argv);
	}
	...
	string lfname, pastr, cabstr, rhoqstr, eps_c_str, gml_fname("tmp.gml"), afname, spm_fname, cab_ifname, cab_ofname, marginal_fname, Aij_fname;
	char **myargv;
	...
	vector<double> _rhoq;
	parse_command_line(argc, argv, lfname, learning_rate, dumping_rate, bp_conv_crit, learning_conv_crit, time_conv, Q, randseed, learning_max_time, pastr, cabstr, rhoqstr, infer_mode, eps_c_str, init_flag, N, afname, vflag, gml_fname, spm_fname, cab_ifname, marginal_fname, no_learn_pa, wblock, wstep, Aij_fname, spcmode, dc_flag, LARGE_DEGREE, cab_ofname, genmode);
	if (dc_flag > 0)
		cout << "DEGREE CORRECTED BLOCK MODEL, mode " << dc_flag << endl;
	else
		cout << "VANILLA BLOCK MODEL" << endl;
	...
	if (!cab_ifname.empty())
	{ // set pa, cab, and rhoq from file named cab_ifname.
		read_cab_from_file(cab_ifname, pa, cab, _rhoq, Q);
	}
	else if (!eps_c_str.empty())
	{ // epsilon,c mode. Set by -P parameter, and
		if (genmode == 1)
		{
		...
		vector<string> tmprhoq = strsplit(rhoqstr, ",");
		for (unsigned int i = 0; i < tmprhoq.size(); i++)
			_rhoq.push_back(atof(tmprhoq[i].c_str()));
	...
	else if (!pastr.empty() && !cabstr.empty() && !rhoqstr.empty())
	{ // pa and cab are set by -p and -c parameter, rhoq set by -a param.
	...
	{ // generation
		bm.graph_gen_graph(N, pa, cab, _rhoq);
		bm.graph_write_gml(gml_fname.c_str());
		if (!spm_fname.empty())
			bm.graph_write_spm(spm_fname.c_str());
		if (!Aij_fname.empty())
			bm.graph_write_A(Aij_fname.c_str());
	}
	else if (function == string("infer"))
	{ // inference
		assert(!lfname.empty());
		bm.graph_read_graph(lfname);
		bm.bm_allocate();
		bm.bm_init(pa, cab, _rhoq);
		bm.do_inference(infer_mode, init_flag, bp_conv_crit, time_conv, dumping_rate);
		if (!marginal_fname.empty())
			bm.output_marginals(marginal_fname);
	}
	else if (function == string("learn"))
	{ // learning
		assert(!lfname.empty());
		if (no_learn_pa)
			cout << "learn only cab !" << endl;
		bm.graph_read_graph(lfname);
		bm.bm_allocate();
		if (!eps_c_str.empty() && dc_flag == 1)
		{ // epsilon,c mode. Set by -P parameter
	...
	else if (function == string("spc"))
	{
		...
		bm.bm_allocate();
		if (spm_fname.empty())
			spm_fname = lfname + string(".spm");
		if (!ifstream(spm_fname.c_str()))
			bm.graph_write_spm(spm_fname.c_str());
