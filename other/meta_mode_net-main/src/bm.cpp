...
void blockmodel::set_Q(int Q_)
{
	Q = Q_;
	init_perms();
}
void blockmodel::set_Q_meta(int Q_meta_) { Q_meta = Q_meta_; }
...
void blockmodel::graph_read_graph(string fname)
{
	...
	string snode = string("node"), sbegin = string("["), send = string("]"), sedge = string("edge"), sid = string("id"), svalue = string("value"), smeta = string("meta"), ssource = string("source"), starget = string("target");
	map<string, int> id2idx;
	map<string, string> id2value;
	map<string, string> id2meta;
	string tmpstr;
	...
	map<string, int> meta2color;
	cout << "reading " << fname << "... " << flush;
	assert(fin.good() && "I can not open the file that contains the graph.");
	int idx = 0, color_idx = 0, metacolor_idx = 0;
	while (fin >> tmpstr)
	{ // read something if it is possible to read all nodes and build node structures.
		if (tmpstr == snode)
		{ // node section begins, let's read id and value.
			fin >> tmpstr;
			assert(tmpstr == sbegin && "[ should follow node");
			bool endnode = false;
			string myid("not_set");
			for (int readtimes = 0; readtimes < 100; readtimes++)
			{ // read at most 100 times to scan all parameters of this node
				fin >> tmpstr;
				if (tmpstr == send)
				{
					endnode = true;
				...
				else if (tmpstr == smeta)
				{ // read a metadata value to an id
					assert(myid != string("not_set") && "id should be given before meta value");
					fin >> tmpstr;
					id2meta[myid] = tmpstr;
					if (meta2color.count(tmpstr) == 0)
					{
						vector<int> tmpvec;
						tmpvec.clear();
						groups_meta.push_back(tmpvec);
						meta2color[tmpstr] = metacolor_idx++;
					}
				}
				else
				{
	...
	int i = -1, j = -1;
	while (fin >> tmpstr)
	{ // read again the file for edges and to build edge structrues.
		if (tmpstr == sedge)
		{ // edge section begins, let's read pairs of nodes
			fin >> tmpstr;
			assert(tmpstr == sbegin && "[ should follow edge");
			fin >> tmpstr;
			while (tmpstr != ssource)
				fin >> tmpstr;
	...
	conf_meta.resize(N);	// metadata labels / assignments
	...
	for (map<string, string>::iterator it = id2meta.begin(); it != id2meta.end(); ++it)
	{
		int idx = id2idx[it->first];
		int color = meta2color[it->second];
		conf_meta[idx] = color;
		groups_meta[color].push_back(idx);
	}
	graph_max_degree = -50;
	for (int i = 0; i < N; i++)
	{
		if (int(graph_neis[i].size()) >= graph_max_degree)
			graph_max_degree = graph_neis[i].size();
	}
	if (bm_dc == 1)
	{
		for (int i = 0; i < N; i++)
			graph_di[i] = 1.0 * graph_neis[i].size() / graph_max_degree;
	}
	else if (bm_dc == 2)
	{
		for (int i = 0; i < N; i++)
			graph_di[i] = 1.0 * graph_neis[i].size();
	}
	else
	{
		for (int i = 0; i < N; i++)
...
void blockmodel::graph_gen_graph(int N_, vector<double> pa_, vector<double> cab_, vector<double> rhoq_)
{
	...
	for (int q = 0; q < Q; q++)
		groups_true[q].resize(0);
	groups_meta.resize(Q_meta);
	for (int q = 0; q < Q_meta; q++)
		groups_meta[q].resize(0);
	graph_neis.resize(N);
	conf_true.resize(N);
	conf_meta.resize(N);

	bm_allocate();
	bm_init(pa_, cab_, rhoq_); // init na, eta, pa, cab, and rhoq
	...
	for (int i = 0; i < N; i++)
	{
		conf_meta[i] = sample_from_categorical(rhoq[conf_true[i]]);
	}
	for (int i = 0; i < N; i++)
	{
		groups_true[conf_true[i]].push_back(i);
		groups_meta[conf_meta[i]].push_back(i);
	}
	graph_edges.clear();
	for (int q1 = 0; q1 < Q; q1++)
	{
		for (int q2 = q1; q2 < Q; q2++)
		{
			int numlinks;
			if (q1 != q2)
				numlinks = int(pab[q1][q2] * na[q1] * na[q2]);
			else
				numlinks = int(pab[q1][q2] * na[q1] * (na[q1] - 1) / 2);
	...
	int num_link_tot = 0;
	for (int i = 0; i < N; i++)
		num_link_tot += graph_neis[i].size();
	num_link_tot /= 2;
	M = graph_edges.size();
...
{
	// warning, think this may no longer be legitimate gml format
	ofstream fout(fname);
...
void blockmodel::bm_allocate()
{
	...
	rhoq_true.resize(Q_true); // prob of node in group q having meta label r
	rhoq_expect.resize(Q);	  // prob of node in group q having meta label r update at each timestep
	...
	cab_expect.resize(Q);
	rhoq.resize(Q); // prob of node in group q having meta label r (param)
	logrhoq.resize(Q);
	...
	for (int q = 0; q < Q; q++)
	{
		...
		rhoq[q].resize(Q_meta);
		logrhoq[q].resize(Q_meta);
		rhoq_expect[q].resize(Q_meta);
	}
	...
	for (int q = 0; q < Q_true; q++)
	{
		...
		rhoq_true[q].resize(Q_meta);
	}
}
//}}}
//{{{void blockmodel:: bm_init(vector<double> pa_, vector<double > cab_)
void blockmodel::bm_init(vector<double> pa_, vector<double> cab_, vector<double> rhoq_)
{
	// Init values for na[Q], eta[Q], pab[Q][Q], cab[Q][Q], and rhoq[Q][Q] from pa_, cab_, and rhoq_
	assert((pa_.size() == Q - 1 || pa_.size() == Q) && "there is something wrong in setting pa, are you sure you set the pa and cab properly?");
	assert((rhoq_.size() == Q) && "there is something wrong in setting rhoq, are you sure you set the rhoq properly?");
	...
	for (int q = 0; q < Q; q++)
	{
		for (int r = 0; r < Q_meta; r++)
		{
			if (q == r)
				rhoq[q][r] = rhoq_[q];
			else
				rhoq[q][r] = (1.0 - rhoq_[q]) / (Q - 1);
			logrhoq[q][r] = log(rhoq[q][r]);
		}
	}
	na[Q - 1] = N - tot_size;
...
void blockmodel::bm_init_random()
{
	// Init values for na[Q], eta[Q], pab[Q][Q], cab[Q][Q], and rhoq[Q][Q] randomly
	vector<double> pa_;
	for (int q = 0; q < Q; q++)
		pa_.push_back(1.0 / Q);
	int tot_size = 0;
	...
	vector<double> cab_;
	for (int q = 0; q < (Q + 1) * Q * 0.5; q++)
		cab_.push_back(FRANDOM);
	int num = 0;
	for (int i = 0; i < Q; i++)
	{
		for (int j = i; j < Q; j++)
...
void blockmodel::bm_rescale_cab()
{
	...
	for (int q = 0; q < Q; q++)
	{
		...
		{
			if (bm_dc == 1)
				cab[q][t] = 2.0 * cab[q][t] / cx * graph_max_degree * N / M * N / M * graph_max_degree / 4.0;
			else if (bm_dc == 2)
				cab[q][t] = 2.0 * N / M * N / M * cab[q][t] / cx / 4.0;
			else
				cab[q][t] = 2.0 * M / N * cab[q][t] / cx;
			cab[t][q] = cab[q][t];
		}
	}
}
...
void blockmodel::bm_init_uniform()
{
	// Uniformly init values for na[Q], eta[Q], pab[Q][Q], cab[Q][Q], and rhoq[Q][Q]
...
void blockmodel::bm_build_na_cab_from_conf()
{
	cout << "building na, cab, and rhoq from configuration..." << flush;
	for (int q = 0; q < Q; q++)
	{
		na[q] = 0;
		for (int t = 0; t < Q; t++)
	...
	for (int i = 0; i < N; i++)
	{
		na[conf_infer[i]] += 1.0;
		if (bm_dc == 1)
			nna[conf_infer[i]] += graph_di[i];
		else if (bm_dc == 2)
			nna[conf_infer[i]] += graph_di[i];
		rhoq[conf_infer[i]][conf_meta[i]] += 1.0;
	}
	for (int e = 0; e < M; e++)
	{
		...
		if (i != j)
			cab[conf_infer[j]][conf_infer[i]] += 1.0; // now cab is the number of edges between group a and b
	}
	for (int q = 0; q < Q; q++)
	{
		eta[q] = 1.0 * na[q] / N;
		...
		{
			if (bm_dc == 1)
				cab[q][t] = cab[q][t] / nna[q] / nna[t] * N;
			else if (bm_dc == 2)
				cab[q][t] = cab[q][t] / nna[q] / nna[t] * N;
			else
				cab[q][t] = cab[q][t] / na[q] / na[t] * N;
			pab[q][t] = cab[q][t] / N;
...
void blockmodel::init_random_number_generator(int seed_)
{
	...
	ip2 = ip - 55;
	ip3 = ip - 61;
	for (unsigned i = ip3; i < ip; i++)
		ira[i] = init_rand4init();
}
//}}}
// sampling from categorical distribution
//{{{int blockmodel:: sample_from_categorical(vector<double> prob)
int blockmodel::sample_from_categorical(vector<double> prob)
{
	double sum = 0.0;
	for (int i = 0; i < prob.size(); i++)
		sum += prob[i];
	assert((fabs(sum - 1.0) < 1e-6) && "probabilities do not sum to 1");
	double r = FRANDOM;
	double cum = 0.0;
	for (int i = 0; i < prob.size(); i++)
	{
...
double blockmodel::compute_overlap()
{
	...
	for (int q = 0; q < Q; q++)
		connect += pow((double)(na_expect[q]) / N, 2);
	for (int pe = 0; pe < perms.size(); pe++)
	{
		double ov = 0.0;
		for (int i = 0; i < N; i++)
			ov += (real_psi[i][perms[pe][conf_true[i]]]);
		ov /= N;
#ifdef OVL_NORM
...
double blockmodel::compute_argmax_energy()
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		{
			if (bm_dc == 2)
				term[q] -= graph_di[i] * pab[q][conf_infer[i]];
			else if (bm_dc == 1)
				term[q] -= graph_di[i] * pab[q][conf_infer[i]];
			else
				term[q] += log(1.0 - pab[q][conf_infer[i]]);
		}
	}
	double energy = 0.0;
	for (int i = 0; i < N; i++)
	{
		...
		{ // contribution of metadata
			if (r == conf_meta[i])
				energy += log(rhoq[qi][r]);
		}
		for (int indexji = 0; indexji < graph_neis[i].size(); indexji++)
		{ // contribution of edges
			int j = graph_neis[i][indexji];
			int qj = conf_infer[j];
			if (bm_dc == 2)
				energy += 0.5 * log(graph_di[i] * graph_di[j] * pab[qi][qj] / (1.0 + graph_di[i] * graph_di[j] * pab[qi][qj]));
			else if (bm_dc == 1)
				energy += log(graph_di[i] * graph_di[j] * cab[qi][qj]) * 0.5;
			else
				energy += log(cab[qi][qj]) * 0.5;
		}
		if (bm_dc == 2)
			energy += 0.5 * graph_di[i] * term[qi];
		else if (bm_dc == 1)
			energy += 0.5 * graph_di[i] * term[qi];
		else
			energy += 0.5 * term[qi]; // contribution of non-edges
	}
	if (bm_dc == 2)
		return -1.0 * (energy + M * log(N)) / N;
	else
		return -1.0 * energy / N;
}
//}}}
...
double blockmodel::compute_config_ovl()
{
	...
	for (int pe = 0; pe < perms.size(); pe++)
	{
		...
		for (int j = 0; j < N; j++)
		{
			if (conf_true[j] == perms[pe][conf_infer[j]])
				ov += 1.0;
		}
		ov /= N;
		if (ov > max_ov)
...
void blockmodel::compute_conductance()
{
	...
	for (int q = 0; q < Q; q++)
	{
		if (bm_conductance[q] < graph_min_conductance)
			graph_min_conductance = bm_conductance[q];
...
double blockmodel::compute_overlap_marg()
{
	...
	double ov = 0.0;
	for (int i = 0; i < N; i++)
		ov += argmax_marginals[i];
	ov /= N;
	double connect = 0.0;
	for (int q = 0; q < Q; q++)
		connect += pow((double)(na[q]) / N, 2);
#ifdef OVL_NORM
	ov = (ov - connect) / (1 - connect);
#endif
	return ov;
...
double blockmodel::compute_na_expect()
{
	...
	for (int i = 0; i < N; i++)
	{
		for (int q = 0; q < Q; q++)
		{
			...
			if (bm_dc == 1)
				nna_expect[q] += graph_di[i] * real_psi[i][q];
			else if (bm_dc == 2)
				nna_expect[q] += graph_di[i] * real_psi[i][q];
		}
	}
...
double blockmodel::bp_compute_f()
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		for (int j = 0; j < graph_neis[i].size(); j++)
			normtot_psi[j] = 0;
		for (int q = 0; q < Q; q++)
		{
			double a = 0.0;
			for (int l = 0; l < graph_neis[i].size(); l++)
			{
				...
				for (int t = 0; t < Q; t++)
				{
					...
					else if (bm_dc == 2)
					{
						...
						b += cab[t][q] * psi[i][l][t]; // sum over messages from l -> i
				}
				a += log(b);
			}
			if (bm_dc == 1)
				pom_psi[q] = a + logeta[q] - di * h[q] / N;
			else if (bm_dc == 2)
				pom_psi[q] = a + logeta[q] - di * h[q] / N;
			else
				pom_psi[q] = a + logeta[q] - h[q] / N;
			// add meta contrib
			pom_psi[q] += logrhoq[q][conf_meta[i]];
			if (pom_psi[q] > maxpom_psi)
				maxpom_psi = pom_psi[q];
		}
		double normtot = 0;
		for (int q = 0; q < Q; q++)
			normtot += exp(pom_psi[q] - maxpom_psi);
		...
		for (int l = 0; l < graph_neis[i].size(); l++)
		{
			...
			for (int q1 = 0; q1 < Q; q1++)
			{
				for (int q2 = q1; q2 < Q; q2++)
				{
					...
					{
						if (bm_dc == 1)
							norm_L += di * dl * cab[q1][q2] * (psi[i][l][q1] * psi[i2][l2][q2]);
						...
						else if (bm_dc == 2)
						{
							...
							norm_L += cab[q1][q2] * (psi[i][l][q1] * psi[i2][l2][q2]);
					}
					else
					{
						if (bm_dc == 1)
							norm_L += di * dl * cab[q1][q2] * (psi[i][l][q1] * psi[i2][l2][q2] + psi[i][l][q2] * psi[i2][l2][q1]);
						...
						else if (bm_dc == 2)
						{
							...
							norm_L += cab[q1][q2] * (psi[i][l][q1] * psi[i2][l2][q2] + psi[i][l][q2] * psi[i2][l2][q1]);
					}
				}
			}
			f_link += log(norm_L);
	...
	for (int q1 = 0; q1 < Q; q1++)
	{
		for (int q2 = q1; q2 < Q; q2++)
		{
			...
			{
				if (bm_dc == 1)
					last_term += 0.5 * cab[q1][q2] * nna_expect[q1] / N * nna_expect[q2] / N;
				else if (bm_dc == 2)
					last_term += 0.5 * pab[q1][q2] * nna_expect[q1] * nna_expect[q2] / N;
				else
					last_term += 0.5 * cab[q1][q2] * na_expect[q1] / N * na_expect[q2] / N;
			}
			else
			{
				if (bm_dc == 1)
					last_term += cab[q1][q2] * nna_expect[q1] / N * nna_expect[q2] / N;
				else if (bm_dc == 2)
					last_term += pab[q1][q2] * nna_expect[q1] * nna_expect[q2] / N;
				else
					last_term += cab[q1][q2] * na_expect[q1] / N * na_expect[q2] / N;
			}
		}
	}
...
void blockmodel::bp_init_h()
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		for (int q1 = 0; q1 < Q; q1++)
		{
			...
			{
				if (bm_dc == 1)
					h[q1] += graph_di[i] * cab[q2][q1] * real_psi[i][q2]; // degree corrected model
				else if (bm_dc == 2)
					h[q1] += graph_di[i] * cab[q2][q1] * real_psi[i][q2]; // degree corrected model
				else
					h[q1] += cab[q2][q1] * real_psi[i][q2];
			}
		}
	}
	for (int q = 0; q < Q; q++)
...
void blockmodel::bp_init_m(double pp)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		{
			if (q == conf_true[i])
				real_psi[i][q] = pp + (1.0 - pp) * FRANDOM;
			else
				real_psi[i][q] = FRANDOM * (1.0 - pp);
		}
		for (int idxij = 0; idxij < graph_neis[i].size(); idxij++)
		{
			for (int q = 0; q < Q; q++)
			{
				if (q == conf_true[i])
					psi[i][idxij][q] = pp + (1.0 - pp) * FRANDOM;
				else
					psi[i][idxij][q] = FRANDOM * (1.0 - pp);
			}
		}
	}
	bp_init_h();
...
void blockmodel::bp_init(int init_flag)
{
	for (int i = 0; i < N; i++)
	{
		...
		{ // initialize messages by planted conf_true
			for (int q = 0; q < Q; q++)
			{
				if (q == conf_true[i])
					real_psi[i][q] = 1.0;
		...
		{ // do not reinit, should be used in learning
		}
	}
	if (vflag > 1)
	{
		if (init_flag == 2)
			cout << "init from true configuration" << endl;
...
double blockmodel::bp_iter_update_psi(int i, double dumping_rate)
{
	...
	for (q = 0; q < Q; q++)
	{
		...
		if (bm_dc == 1)
			pom_psi[q] = a * eta[q] * exp(-1.0 * di * h[q] / N);
		else if (bm_dc == 2)
			pom_psi[q] = a * eta[q] * exp(-1.0 * di * h[q] / N);
		else
			pom_psi[q] = a * eta[q] * exph[q];
		// add meta contrib
		pom_psi[q] *= rhoq[q][conf_meta[i]];
		normtot_real += pom_psi[q];
		for (l = 0; l < graph_neis[i].size(); l++)
		{
			if (field_iter[l] < EPS)
			{ // to cure the problem that field_iter[l] may be very small
				double tmprob = 1.0;
	...
	for (int q1 = 0; q1 < Q; q1++)
	{
		...
		{
			if (bm_dc == 1)
				h[q1] -= di * cab[q2][q1] * real_psi[i][q2];
			else if (bm_dc == 2)
				h[q1] -= di * cab[q2][q1] * real_psi[i][q2];
			else
				h[q1] -= cab[q2][q1] * real_psi[i][q2];
...
double blockmodel::bp_iter_update_psi_large_degree(int i, double dumping_rate)
{
	...
	for (q = 0; q < Q; q++)
	{
		...
		if (bm_dc == 1)
			pom_psi[q] = a + logeta[q] - 1.0 * di * h[q] / N;
		else if (bm_dc == 2)
			pom_psi[q] = a + logeta[q] - 1.0 * di * h[q] / N;
		else
			pom_psi[q] = a + logeta[q] - h[q] / N;
		...
		{
			psii_iter[q][l] = pom_psi[q] - field_iter[l];
			if (psii_iter[q][l] > maxpom_psii_iter[l])
				maxpom_psii_iter[l] = psii_iter[q][l];
		}
	}
	for (q = 0; q < Q; q++)
	{
		...
		for (l = 0; l < graph_neis[i].size(); l++)
			normtot_psi[l] += exp(psii_iter[q][l] - maxpom_psii_iter[l]);
	}
	for (int q1 = 0; q1 < Q; q1++)
	{
		for (int q2 = 0; q2 < Q; q2++)
...
int blockmodel::bp_converge(double bp_err, int max_iter_time, int init_flag, double dumping_rate)
{
	...
	for (iter_time = 0; iter_time < max_iter_time; iter_time++)
	{
		...
		{
			if (vflag >= 1)
				cout << " bp-:) iter_time=" << iter_time << " " << endl;
			if (vflag >= 10)
				show_marginals(vflag);
			else if (vflag >= 3)
				show_marginals(10);
	...
	if (vflag >= 1)
		cout << " bp-:( iter_time=" << max_iter_time << " " << endl;
	if (vflag >= 3)
		show_marginals(10);
	//			fout.close();
	return -1;
		...
		for (int l = 0; l < graph_neis[i].size(); l++)
		{
			...
			for (int q1 = 0; q1 < Q; q1++)
			{
				for (int q2 = q1; q2 < Q; q2++)
				{
					...
					{
						if (bm_dc == 1)
							cab_expect[q1][q2] += 0.5 * di * dl * cab[q1][q2] * (psi[i][l][q1] * psi[i2][l2][q2]) / norm_L;
						else if (bm_dc == 2)
						{
							double tmp = di * dl * pab[q1][q2];
							...
							cab_expect[q1][q2] += 0.5 * cab[q1][q2] * (psi[i][l][q1] * psi[i2][l2][q2]) / norm_L;
					}
					else
					{
						if (bm_dc == 1)
							cab_expect[q1][q2] += 0.5 * di * dl * cab[q1][q2] * (psi[i][l][q1] * psi[i2][l2][q2] + psi[i][l][q2] * psi[i2][l2][q1]) / norm_L;
						else if (bm_dc == 2)
						{
							double tmp = di * dl * pab[q1][q2];
							...
							cab_expect[q1][q2] += 0.5 * cab[q1][q2] * (psi[i][l][q1] * psi[i2][l2][q2] + psi[i][l][q2] * psi[i2][l2][q1]) / norm_L;
					}
				}
			}
		}
	...
	for (int q1 = 0; q1 < Q; q1++)
	{
		for (int q2 = q1; q2 < Q; q2++)
		{
			if ((na_expect[q1] > EPS) && (na_expect[q2] > EPS))
			{
				...
				{
					if (bm_dc == 1)
						cab_expect[q1][q2] *= N / (nna_expect[q1] * nna_expect[q2]);
					else if (bm_dc == 2)
						cab_expect[q1][q2] *= N / (nna_expect[q1] * nna_expect[q2]);
					else
				...
				{
					if (bm_dc == 1)
						cab_expect[q1][q2] *= 2. * N / (nna_expect[q1] * nna_expect[q2]);
					else if (bm_dc == 2)
						cab_expect[q1][q2] *= 2. * N / (nna_expect[q1] * nna_expect[q2]);
					else
						cab_expect[q1][q2] *= 2. * N / (na_expect[q1] * na_expect[q2]);
				}
			}
		}
	}
...
//}}}
//{{{double blockmodel:: bp_compute_rhoq_expect()
double blockmodel::bp_compute_rhoq_expect()
{
	// first zero rhoq_expect
	for (int q = 0; q < Q; q++)
	{
		for (int r = 0; r < Q_meta; r++)
			rhoq_expect[q][r] = 0.0;
	...
	for (int q = 0; q < Q; q++)
	{
		for (int i = 0; i < N; i++)
		{
			rhoq_expect[q][conf_meta[i]] += real_psi[i][q];
		}
		double rhoq_norm = 0.0;
		for (int r = 0; r < Q_meta; r++)
			rhoq_norm += rhoq_expect[q][r];
		for (int r = 0; r < Q_meta; r++)
			rhoq_expect[q][r] /= rhoq_norm;
	}
	return 1.0;
}
//}}}
...
int blockmodel::nmf_converge(double nmf_err, int max_iter_time, int init_flag, double dumping_rate)
{
	...
	for (int iter_time = 0; iter_time < max_iter_time; iter_time++)
	{
		...
		{
			if (vflag >= 1)
				cout << " MF-:) iter_time=" << iter_time << " " << endl;
			if (vflag >= 3)
				show_marginals(10);
			return iter_time;
		}
	...
	if (vflag >= 1)
		cout << " MF-:( iter_time=" << max_iter_time << " " << endl;
	if (vflag >= 3)
		show_marginals(10);
	return -1;
}
...
double blockmodel::nmf_iterate(int i, double dumping_rate)
{
	...
	for (int q = 0; q < Q; q++)
	{
		...
		for (int idxij = 0; idxij < graph_neis[i].size(); idxij++)
		{
			...
			for (int t = 0; t < Q; t++)
				pom_psi[q] += (logcab[q][t] + pab[q][t]) * real_psi[j][t];
		}
	...
	for (int i = 0; i < N; i++)
	{
		for (int a = 0; a < Q; a++)
		{
			...
			for (int idxij = 0; idxij < graph_neis[i].size(); idxij++)
			{
				int j = graph_neis[i][idxij];
				for (int b = 0; b < Q; b++)
					tmp += (logcab[a][b] + pab[a][b]) * real_psi[j][b];
			}
	...
	for (int i = 0; i < N; i++)
	{
		for (int l = 0; l < graph_neis[i].size(); l++)
		{
			...
			for (int q1 = 0; q1 < Q; q1++)
			{
				...
				{
					if (q1 == q2)
						norm_L += cab[q1][q2] * (real_psi[i][q1] * real_psi[i2][q2]);
					else
						norm_L += cab[q1][q2] * (real_psi[i][q1] * real_psi[i2][q2] + real_psi[i][q2] * real_psi[i2][q1]);
				}
			}
			for (int q1 = 0; q1 < Q; q1++)
			{
				...
				{
					if (q1 == q2)
						cab_expect[q1][q2] += 0.5 * cab[q1][q2] * (real_psi[i][q1] * real_psi[i2][q2]) / norm_L;
					else
						cab_expect[q1][q2] += 0.5 * cab[q1][q2] * (real_psi[i][q1] * real_psi[i2][q2] + real_psi[i][q2] * real_psi[i2][q1]) / norm_L;
				}
			}
		}
	}
...
void blockmodel::EM_learning(int mode, int init_flag, double conv_crit, int time_conv, double dumping_rate, double learning_rate, int learning_max_time, double learning_conv_crit, bool no_learn_pa)
{
	...
	for (learning_time = 0; learning_time < learning_max_time; learning_time++)
	{
		...
		if (vflag >= 0)
		{
			...
			if (groups_true.size() > 0)
				cout << compute_overlap_fraction() << deli;
			cout << fdiff << deli << conv_crit << deli;
			// cout<<fdiff<<deli;
		}
		if (isnan(fold) || isinf(fold))
			break;
		if (vflag >= 0)
		{
			for (int q = 0; q < Q; q++)
				cout << (double)(na_expect[q]) / (double)(N) << " ";
			cout << endl;
		}
...
void blockmodel::EM_step(double learning_rate, bool no_learn_pa)
{
	...
	for (int i = 0; i < Q; i++)
	{
		...
		for (int j = 0; j < Q_meta; j++)
		{
			rhoq[i][j] = learning_rate * rhoq_expect[i][j] + (1.0 - learning_rate) * rhoq[i][j];
			logrhoq[i][j] = log(rhoq[i][j]);
		}
	}
}
...
void blockmodel::output_f_ovl(int mode)
{
	...
	if (groups_true.size() > 0 && groups_true.size() == Q)
		cout << "overlap=" << compute_overlap() << deli << "fraction=" << compute_overlap_fraction() << endl;
	else
		cout << endl;
}
//}}}
...
void blockmodel::output_marginals(string fname)
{
	...
	if (groups_true.size() > 0)
		fout << "overlap=" << compute_overlap() << deli << "fraction=" << compute_overlap_fraction() << endl;
	else
		fout << endl;
	compute_argmax_conf();
...
void blockmodel::spec(string fname, string origin_fname, int mode)
{
	ostringstream cmd;
	if (mode != 4)
		cmd << "spc.py " << N << " " << fname << " " << Q << " " << mode;
	else
		cmd << "mspc.py " << origin_fname << " " << Q;
	if (vflag >= 1)
		cout << cmd.str() << endl;
