...
public:
	...
	// graph part
	int N, Q, Q_true, Q_meta, M, graph_max_degree;
	// N: number of nodes.  Q: number of colors of the model!!!   Q_true: number of colors given. Q_meta: number of meta labels
	...
	vector<vector<int>> groups_true, groups_infer, groups_meta;
	// groups_true is set of Q vecs containing idxs of nodes in each group
	vector<int> conf_true, conf_infer, conf_meta;
	// conf_true stores configuration(assignment, or colors or communities) that are given
	// conf_infer stores configuration that is inferred
	// conf_meta stores configuration of metadata
	vector<double> graph_di, bm_conductance; // degree of nodes, normalized by maximum degree.
	double graph_min_conductance;

	// block model
	// added params here for metadata - rhoq for prob of metadata labelling true group
	// for node in group q, else w prob 1-rhoq it is uniformly sampled from any other group
	...
	// eta (alpha for us) is prior over groups, which corresponds to na / N
	// however, in practice don't know which the 'true' group is, so need to learn
	// categorical distribution over groups, rhoq, for each group q, ie
	// rhoq[q][r] is the prob that a node in group q has metadata label of group r
	vector<vector<double>> pab, cab, logcab, cab_true, pab_true, cab_expect, rhoq, logrhoq, rhoq_true, rhoq_expect;
	// pab[q][r] is prob of edge between nodes in group q and r, which here
	// corresponds to cab[q][r] / N
	int bm_dc;
	...
	int sample_from_categorical(vector<double>);
	// graph
	...
	void graph_gen_graph(int, vector<double>, vector<double>, vector<double>);
	// block model
	blockmodel();
	void set_Q(int);
	void set_Q_meta(int);
