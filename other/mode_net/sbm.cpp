...
int main(int argc, char** argv)
{
	...
	if(argc > 1){
		...
		else if(function != string("gen") && function != string("infer") && function != string("learn")&& function != string("walk")&& function != string("spc") && function != string("learnq")) show_help_short(argv);
	}
	...
	if(function == string("gen")){ // generation
		...
		bm.bm_show_na_cab();
		if(!marginal_fname.empty()) bm.output_marginals(marginal_fname);
	} else if(function == string("learnq")){
		// TODO @Jiaze to learn the number of communities. Set it to Q and return. 
		assert(!lfname.empty());
		bm.graph_read_graph(lfname);
		ofstream fout(marginal_fname.c_str());
		unsigned int q;
		double f_q;
		// q from 2 to 8, find the smallest q that make the free energy dont decrease
		for(q=2;q<9;q++) {
			bm.set_Q(q);
			bm.bm_allocate();
			double f;  // do some times learning, select min f as free energy in this q.
			for(unsigned int t=0;t<10;t++) {
				if(!eps_c_str.empty() && dc_flag==1){ //epsilon,c mode. Set by -P parameter
					...
					cout<<"bm(matrices) is randomly initialized! for q="<<q<<endl;
					bm.bm_init_random();
				}
				bm.EM_learning(infer_mode,init_flag,bp_conv_crit,time_conv,dumping_rate,learning_rate, learning_max_time, learning_conv_crit,no_learn_pa);
				double fnew = bm.bp_compute_f(); // get free energy
				if (t == 0 || fnew < f ) {
					f = fnew;
				}
				if(!marginal_fname.empty()) {
					fout<<q<<deli<<"times"<<t<<deli<<"free energy"<<f<<endl;
				}
			}
			if(!marginal_fname.empty()) {
				fout<<q<<deli<<f<<endl;
