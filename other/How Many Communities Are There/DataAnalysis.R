...
node.attr = read.csv("TradeAttributes.csv", header = TRUE) 
...
bay.res = which(cand.bay==max(cand.bay))
aux = mixer(A,qmin=bay.res,qmax=bay.res,method="bayesian"); bay.model = getModel.mixer(aux)
c.bay = max.col(t(bay.model$Taus)); 

Membership.Vector = Re.Index.Labels(c.bay,bay.res) 
comm = vector("list",bay.res); comm.bay = vector("list")
for (j in 1:bay.res) comm[[j]]= which(Membership.Vector == j)
comm.bay$Membership.Vector = Membership.Vector
comm.bay$Clusters = comm
...
V(RN)[unlist(comm.bay$Clusters[1])]$color = "mediumseagreen"
V(RN)[unlist(comm.bay$Clusters[2])]$color = "SkyBlue2"
V(RN)[unlist(comm.bay$Clusters[3])]$color = "green"
V(RN)[unlist(comm.bay$Clusters[4])]$color = "darkgoldenrod1"
V(RN)[unlist(comm.bay$Clusters[5])]$color = "salmon"
V(RN)[unlist(comm.bay$Clusters[6])]$color = "yellow"
V(RN)[unlist(comm.bay$Clusters[7])]$color = "cyan"
...
g = scan("School7Arcs.txt",n=-1); totdat = length(g)/3
...
# Read Attributes Data
node.attr = read.csv("School7Attributes.csv", header = TRUE) 
